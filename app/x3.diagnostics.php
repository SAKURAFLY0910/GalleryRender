...
$x3_diagnostics = isset($x3_config) && $x3_config["settings"]["diagnostics"] ? true : false;
...
# Make sure /config/ folder is blocked
$config_blocked = true;
if(file_exists($file_path.'/config/readme.txt') && function_exists('get_headers')) {
	$test_config_readable = @get_headers(ABSOLUTE.'/config/readme.txt');
	if(!empty($test_config_readable) && strpos($test_config_readable[0], '403') === false) {
...
if($mod_rewrite_confirmed) {
	...
	if($server_is_like_apache) {
		if(!file_exists($file_path.'/.htaccess') || isset($_SERVER["X3_SERVER_CONFIG"])) {
			$success .= addItem("success", $server_software . " X3 Config Detected", "Congratulations! It seems you have successfully configured X3 from your " . $server_software . " server config.");
		}

	# Successful server config generic (NGINX, Lighttp etc)
	} else if($server_software !== 'iis'){
		$success .= addItem("success", "Server Config Detected [" . $server_software . "]", "Congratulations! It seems you have successfully configured X3 from your " . $server_software . " server config.");
	}
}
...
$unused_dirs = array('extensions', 'templates', /*'public',*/ 'content/feed', 'content/json', /*'content/services',*/ /*'content/sitemap',*/ 'content/custom/css', 'content/custom/footer', 'content/custom/head', 'content/custom/header', 'content/custom/javascript', 'content/custom/mail', 'content/custom/widget.contact');
...
function apache_check_htaccess($first_check = true, $patching = false){
	...
	# global vars
	global $file_path, $warning, $rewrite_base, $htaccess_allowed, $critical, $success, $link_path, $has_mod_rewrite, $mod_rewrite_working, $mod_rewrite_confirmed, $server_software, $config_blocked, $x3_config, $server_is_like_apache, $x3_diagnostics;
...
# content dir exists
	...
	if(!is_writeable($file_path.'/content')){
	...
	if(count($index_dirs)){
		...
		$index_dir_images = glob($index_dir."/*.jpg", GLOB_NOSORT);
...
if(!empty($session_save_path) && !$open_basedir && strpos($session_save_path, '://') === false && strpos($session_save_path, '/') !== false){
	...
	if(strpos($session_save_path, '/') !== 0) $session_save_path = strstr($s, '/');
...
if($x3_config["settings"]["preload"] && !file_exists($file_path.'/content/site.json')){
...
if(!$mod_rewrite_confirmed){
	...
	# Unsupported server software warning
		...
		$warning .= addItem("warning", $_SERVER['SERVER_SOFTWARE'], "You are running server software not tested with X3, and we cannot guarantee that X3 will work properly. <a href='https://imagevuex.com/contact/' target='_blank'>Email us</a> with details about your specific server software, and we will look into it.");
