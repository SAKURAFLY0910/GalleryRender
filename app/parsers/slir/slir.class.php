...
class SLIR
{
	...
	private function allocateMemory()
	{
		...
		// allocate memory?
		if(SLIRConfig::$allocateMemory) {

			// get memory_limit
			$memory_limit = @ini_get('memory_limit');

			// only proceed if memory_limit < maxMemoryToAllocate
			if(!empty($memory_limit) && intval($memory_limit) > 0 && intval($memory_limit) < SLIRConfig::$maxMemoryToAllocate){

				// always max
				if(SLIRConfig::$alwaysMaxMemory){
				...
				// estimate memory
					...
					// estimate memory requirement with generous margin (fudge)
					$estimatedMemory = min(round(($this->source->width * $this->source->height + $this->rendered->width * $this->rendered->height) / (80000 / SLIRConfig::$memoryFudge)), SLIRConfig::$maxMemoryToAllocate);
					// $memoryNeeded = Round(($imageInfo[0] * $imageInfo[1] * $imageInfo['bits'] * $imageInfo['channels'] / 8 + Pow(2, 16)) * 1.65);

					// up memory_limit if estimatedMemory > $memory_limit
					if($estimatedMemory > $memory_limit) @ini_set('memory_limit', $estimatedMemory . 'M');

					/*// debug
					$estimatedMemory_slir = round((($this->source->width * $this->source->height * 5) / 1024) / 1024, 0) + 15;
					var_dump('estimatedMemory SLIR: ' . $estimatedMemory_slir . '<br>');
					var_dump('estimatedMemory: ' . $estimatedMemory . '<br>');
					var_dump('OLD memory_limit: ' . $memory_limit . 'M<br>');
					var_dump('NEW memory_limit: ' . ini_get('memory_limit') . '<br>');*/
				}
			}
		}
	}
	...
	private function serveHeaders($lastModified, $mimeType, $fileSize, $SLIRHeader)
	{
		...
		// proxy servers.
		header('Cache-Control: public, max-age=' . SLIRConfig::$browserCacheTTL);
		//header('Cache-Control: public, max-age=' . SLIRConfig::$browserCacheTTL . ', immutable');
