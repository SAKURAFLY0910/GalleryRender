...
// INSTRUCTIONS
// 1. Upload x3_installer.php into the directory where you wish to install X3.
	...
	// clean up
		...
		function mkdir_if_missing($path){
			if(!file_exists($path)) {
				...
				if($created) {
					//$output .= addItem('neutral', false, $tick . 'Successfully created new <code>' . $path . '</code> directory.');
					return true;
				} else {
					$output .= addItem('neutral', false, $fail . 'Failed to create new <code>' . $path . '</code> directory.');
					return false;
				}
			} else {
				return true;
